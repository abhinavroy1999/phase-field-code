...
void cahn_hilliard_evolution(double kappa, double A, int time_step, double dt, int Nx, int Ny, double dx, double dy, fftw_complex *comp)
{
  FILE *fw;
  char file_name[50];
 ...
 g = fftw_malloc(Nx*Ny*sizeof(fftw_complex));
  ...
  plan3 = fftw_plan_dft_2d(Nx, Ny, comp, comp, FFTW_BACKWARD, FFTW_ESTIMATE);
  ...
  for (temp = 1; temp <time_step+1; ++temp)
  {
    for(i1=0; i1<Nx; ++i1)
    {
      for(i2=0; i2<Ny; ++i2)
      {
        g[i2+Ny*i1] = 2.0*A*((comp[i2 + i1*Ny]))*(1.0-(comp[i2 + i1*Ny]))*(1.0-2.0*(comp[i2 + i1*Ny]));
      }
    }
    ...
    for(i1=0; i1 < Nx; ++i1)
    {
       ...
       for(i2=0; i2 < Ny; ++i2)
       {
          ...
          { ky = (i2-Ny)*delky; }
	         k2 = kx*kx + ky*ky;
	         k4 = k2*k2;
           denominator = (1.0 + 2.0*kappa*k4*dt);
          comp[i2 + Ny*i1] = (comp[i2+Ny*i1] - 1.0*k2*dt*g[i2 + Ny*i1])/denominator;
        }
    }

    fftw_execute(plan3);
    //Determining the average alloy composition, which is the actual alloy composition.
    for(i1=0; i1<Nx; ++i1)
    {
          for(i2=0; i2<Ny; ++i2)
          {
