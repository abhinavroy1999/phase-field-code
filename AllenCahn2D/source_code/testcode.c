/*  
    Code to perform test on the input data to determing whether the given simulation data is in the acceptable range.
...
void input_data_test(double kappa, double A, int time_step, double dt, int Nx, int Ny, double dx, double dy, double phi_zero, double phi_noise)
{
  FILE *fw;
  int index = 0;
  ...
  if (kappa<=0)
  {
    printf("The value of of the parameter 'kappa' cannot be 0 or negative. Kindly change the value accordingly.\n");
    index=1;
  }
  ...
  if (phi_zero<=0||phi_zero>=1)
  {
    printf("The order parameter should lie between 0 and 1 (excluding the extreme points). Kindly change the value accordingly.\n");
    index=1;
  }
  //Check the value of composition noise strength.
  if (phi_noise>=1)
  {
    printf("The strength of noise in the order parameter should not be greater than 1. Kindly change the value accordingly.\n");
    index=1;
  }
  if (index == 1)
  {
